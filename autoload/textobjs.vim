...
" visually select around fold under cursor
" @arg a:xmap {bool} indicates if called from xmap
" a:xMap true: don't reselect 
function! textobjs#az(xMap)
	try
		if (foldclosed('.') ==# -1)
			normal! zc
		endif
		...
		" if no fold is found, it throws E490
		echohl ErrorMsg
		echon 'No fold found'
		echohl NONE
	endtry
...
" select text before an equals sign
"
" @arg toBeg {bool} if true selection will occupy from first
"                   non-white-space to last non-white-space before the =
"
" @arg xMap {bool} indicates if called from xmap; otherwise, called from
"                  omap
"
" a:toBeg true:  Select from first non-white-space on the line until the
"                last non-white-space before the =.
"
" a:toBeg false: Select from the beginning of the first WORD before the =
"                until the last non-white-space of that WORD. 
"
" a:xMap true: it was called from an xmap
"
" a:xMap false: it was called from an omap
"
" a:xMap is only pertinent when the equals sign is at the beginning of
" the line if. When a:xMap is false (0), and the the equals sign is the
" first non-white-space character on the line, then a space is padded before
" the equals sign; this is to maintain proper formatting.
function! textobjs#beforeEquals(toBeg, xMap)
	let l:line = getline('.')
	...
	if (l:line =~# '^\s*=')
		...
		if (!a:xMap)
			" omaps should, in theory, only select not modify the text; however,
			" if this was called from an xmap, try to maintain appropriate
			" padding (this approach isn't ideal, but it should do the trick for
			" most situations)
			execute "normal! 0f=i  \<esc>hv"
		endif
		return
	endif
	...
	" move to the first non-white-space before the cursor or beg of line
	call search('\S\s*\%'.virtcol('.').'v', 'bc', l:lineNr)

	normal! v

	if (a:toBeg)
		" entire line before equals sign
		normal! ^
		return
	endif

	" move to first non-white-space of current word
	call search('\S*\%'.virtcol('.').'v', 'bc', l:lineNr)
endfunction
...
" this can handle the following three formats (so long as textobjs#regNums is
...
function! textobjs#aroundNumber()
	...
	" create regex pattern matching any binary, hex, decimal number
	let l:pat = join(textobjs#regNums, '\+\|') . '\+'
...
function! textobjs#inIndentation()
...
"
" WARNING: python devs have been known to become addicted to this
function! textobjs#aroundIndentation()
